...
class AsyncMilvusClient(BaseMilvusClient):
    ...
    def get_server_type(self) -> str:
        """Get the server type. Requires the client to be connected."""
        if self._handler is None:
            raise MilvusException(
                message="Client not connected. Call await client._connect() first."
            )
    ...
    async def add_collection_struct_field(
        ...
        **kwargs,
        ...
        conn = await self._get_connection()
        await conn.add_collection_struct_field(
            collection_name,
            struct_field_schema,
            timeout=timeout,
            context=self._generate_call_context(**kwargs),
